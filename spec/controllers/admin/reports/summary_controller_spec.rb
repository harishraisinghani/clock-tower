describe Admin::Reports::SummaryController do
  ...
  describe '#show' do
    ...
    before :each do
      @entries = []
      3.times do
        @entries << create(:time_entry, duration_in_hours: 1)
      end
    end
    ...
    it "should allow assign @time entries to a collection of time entries" do
      ...
      expect(assigns[:time_entries].length).to be(3)
    end

    it "should returns CSV format" do
      get :show, from: Date.today.beginning_of_week.to_s, to: Date.today.to_s, format: 'csv'
